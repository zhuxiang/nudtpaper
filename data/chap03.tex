...
\begin{table}
    \centering
    \caption{Dataset characteristics}\label{tab:dataset}
    \begin{tabular}{ccc}
        \hline
...
在算法的比较方面，我们将所提出的\textit{VecCNN}算法与两个基准算法朴素贝叶斯（\textit{Naive Bayes}）和支撑向量机（\textit{Support Vector Machine}）进行比较。
\section{本章小结}
